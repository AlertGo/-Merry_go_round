...
function Swipe(){
    ...
    this.dots = [];
    this.index = 0;
    this.direction = 1;		// 图片滚动方向，1:左，-1:右， 0:没有移动
    this.swipeTimer = null;
    this.touch = {
        swiping: 0, // 是否正在滑动中
        isCanSwipe: 0, // 是否可以滑动， 图片为一张的时候 :0
        isMoving: 0,	// 是否有移动，用于区别click和touch
        startX: 0,
        startY: 0,
        distance: 0,
        current: 0,
...
Swipe.prototype.initSwipe = function(id){
    ...
    if (this.list.length > 1) {
        ...
        if (dotBox) {
            dotBox.className = 'swipe-dot show'; //定义容器元素样式
...
Swipe.prototype.scroll = function(page) {
    ...
    nextLi.style.left = this.width + 'px';
...
Swipe.prototype.translate = function(element, offset, speed, callback) {
    if (speed) {
        element.style.webkitTransition = 'left ' + speed + 'ms ease';
        element.style.transition = 'left ' + speed + 'ms ease';
        setTimeout(function () {
            offset = -1 * offset;
...
Swipe.prototype.touchEnd = function(e) {
    var _this4 = this;
    var touch = this.touch;
    if (!touch.isCanSwipe || !touch.swiping || !touch.isMoving) return;
    touch.isMoving = !1;
